...
class Books extends CI_Model {
     ...
     * @return object
     */
    function get_bestbook($limit = null) {
	...
	-> order_by ('count(status)','desc');
		...
		//SELECT count(book_id) as aa,book_id FROM `fj_like_dislike` where status =1  group by book_id 
       ...
       /* $this->db
        ...
        if($q->num_rows() > 0) {
            ...
            $this->books->bestbook = $fav_books;
            
            return $this->books->bestbook;
	...
	function likeDislike($data){
        $q = $this->db->select('user_id,book_id')->from('like_dislike')
		...
		if($q->num_rows() > 0){
            ...
            ->where('user_id', $data['user_id'])
			->where('book_id',$data['book_id'])
            ->update('like_dislike',array('status'=>$data['status'])); 
             return refresh;			
		}else{
		      $this->db->insert('like_dislike',$data);  
			   return $this->db->insert_id();
		}		
	     		 
	}
    
	function likeDislikeCounter($nData){
	$q = $this->db->select('SUM(STATUS) as counter')->from('like_dislike')
				->where('book_id',$nData)
				->get();
	return $q->result(); 
	} 
    
	function likeDislikeCounterNew($nDataBook,$nDataUser){
	$q = $this->db->select('status')->from('like_dislike')
				->where('book_id',$nDataBook)
				->where('user_id',$nDataUser)
				->get();
	return $q->result();
	} 
